...
import { buscarItensPorTermo, listarHistorico, listarUltimasMovimentacoes } from "../lib/repository";
import type { HistoricoFiltros, Item, MovimentacaoComItem, Setor } from "../lib/types";

export function useSetores(): Setor[] {
  return useLiveQuery(() => db.setores.orderBy("nome").toArray(), [], []) ?? [];
}
...
export function useUltimasMovimentacoes(limite = 5): MovimentacaoComItem[] {
  return useLiveQuery(() => listarUltimasMovimentacoes(limite), [limite], []) ?? [];
}
